...
# e) Descriptive analysis ----
  ...
  df_count <- as.data.frame(table(sales%>%filter(in_shop==1)%>%select(product_id)))
  ...
  sales%>%filter(in_shop==1)%>%
    ...
    theme_minimal() +  
    theme(axis.text.x = element_text(angle = 45, hjust = 1))+
    labs(title = "Stores that sale the most", x = "Store", y = "Sales quantity")
  ...
  ggplotly(gg) # Smoothing would be better maybe !
  ...
  gg<-ggplot(sales_weekly)+
    ...
    geom_point(data=subset(sales_weekly, !is.na(promo)),aes(x=date, y =sales_quantity,color=product_id,alpha=promo))+
    ggtitle("Sales quantity in function of date (daily) by product with promotion")
  ...
  gg<-ggplot(sales_weekly)+
    ...
    ggtitle("Sales quantity in function of date (weekly) by store with promotion")
  ...
  gg<-ggplot(sales_weekly)+
    geom_line(aes(x=date, y =sales_price, color = product_id))+
    geom_point(data=subset(sales_weekly, !is.na(promo)),aes(x=date, y =sales_price,color=product_id,alpha=promo))+
    ggtitle("Sales price in function of date (daily) by product")
  ...
  sales_weekly <- sales%>%filter(in_shop==1)%>%
    ...
    geom_point(data=subset(sales_weekly, !is.na(promo_bin_1)),aes(x=date, y =price,color=store_id,alpha=promo_numeric))
  ...
  ## Analysis of the promotion ##
     ...
     # Product which is the most sold +  Less expensive product :
  ggplotly(sales%>%filter(in_shop==1)%>%
             ...
             theme_minimal() +  
             theme(axis.text.x = element_text(angle = 45, hjust = 1))+
             labs(title = "Stores promotion for product P0051 for 2017-2019", x = "Date", y = "Number of store" ) )

    # Most expensive product
  ggplotly(sales%>%filter(in_shop==1)%>%
             filter(product_id=="P0015")%>%
             group_by(promo_bin_1,date)%>%
             summarise(n_store_promo=n())%>%
             ggplot()+
             geom_bar(aes(x=date,y=n_store_promo, fill=promo_bin_1),stat = "identity")+
             theme_minimal() +  
             theme(axis.text.x = element_text(angle = 45, hjust = 1))+
             labs(title = "Stores promotion for product P0085 for 2017-2019", x = "Date", y = "Number of store" ) )
  

  # Less sold product
  ggplotly(sales%>%filter(in_shop==1)%>%
             filter(product_id=="P0014")%>%
             group_by(promo_bin_1,date)%>%
             summarise(n_store_promo=n())%>%
             ggplot()+
             geom_bar(aes(x=date,y=n_store_promo, fill=promo_bin_1),stat = "identity")+
             theme_minimal() +  
             theme(axis.text.x = element_text(angle = 45, hjust = 1))+
             labs(title = "Stores promotion for product P0014 for 2017-2019", x = "Date", y = "Number of store" ) )
...
# II - Data smoothing ----
  ...
  ggplot(df_p51_s2)+
    geom_point(aes(x=date,y=sales))+
    geom_line(aes(x=date,y=sales_pred),color="red",linewidth=1)+
    ggtitle("Smoothing of sales product P0051 for store S0002")# We have negative values which is not desirable, we replace them:
  ...
  ggplot(df_p51_s2)+
    geom_point(aes(x=date,y=sales))+
    geom_line(aes(x=date,y=sales_pred),color="red",linewidth=1)+
    ggtitle("Smoothing of sales product P0051 for store S0002 without negative values") #better.
  ...
  ggplot(df_p51_s2)+
    geom_point(aes(x=date,y=sales))+
    geom_line(aes(x=date,y=sales_pred),color="red",linewidth=1)+
    ggtitle("Smoothing of sales product P0051 for store S0002 without negative values optimized")#better.
  ...
  ggplotly(ggplot(product_data_51)+
    ...
    ggtitle("Smoothed sales for product P0051 by store"))
  ...
  grid.arrange(grobs=Plot_list[sample(1:144,12)])
  ...
  rm(basismat, df_sales_product51x, fit, smooth_funcs_51, product51_data, Spline_b, bic_values, i, nbasis, nbasis_range, optimal_nbasis, time )
  ...
  rm(smooth_funcs_all, i, j, k, df_sales_productx_storex, bic_values, time, nbasis, Spline_b, basismat, fit, optimal_nbasis, funList)
  ...
  ggplotly(ggplot(Simplified_sales)+
             geom_line(aes(x = date, y = sales_pred, color = product_id)) +
             ggtitle("Smoothed sales quantity in function of date (daily) by product"))
  
  # let's delete all the variables we don't need before moving to the next part
  rm(basismat, df_sales_product51, fit, gg, my_smooth_func_51, product51_data, smooth_func, Spline_b, bic_values,i, j, nbasis, nbasis_range, result_coef, optimal_nbasis, smooth_values)
  ...
  ggplotly(ggplot(df_price_wt_store) +
             ...
             ggtitle("Smoothed Sales price in function of date (daily) by product"))
...
# a) FPCA on total sales quantity by product ----
  ...
  ggplotly(ggplot(Simplified_sales)+
    geom_line(aes(x=date,y=sales_pred,color=product_id))+
      ggtitle("Smoothed Sales by product"))
  ggplotly(ggplot(Simplified_sales)+
             geom_point(aes(x=date,y=total_sales,color=product_id)))
...
# b) FPCA on sales quantity for a product ----
  ...
  ggplotly(ggplot(df_p35)+
             geom_line(aes(x=date,y=sales_pred,color=store_id))+
             ggtitle("Smoothed sales of P0035 by store"))
